...
class Research extends CI_Controller 
{
    ...
    function trend_result() {
        $category = $this->input->post('radioSentiment');
        $limit = $this->input->post('table_data_length');
        $fromDate = $this->input->post('interval_date_from');
        $toDate = $this->input->post('interval_date_until');
        ...
        $sql = 'SELECT v.venue_id, v.venue, c.value,(d.positive-d.negative)/(d.positive+d.negative+d.netral) AS score, d.positive, d.negative, d.netral
                ...
                FROM comment
                GROUP BY venue_id) d,
                comment c INNER JOIN venue v on c.venue_id = v.venue_id
                where d.venue_id = c.venue_id and c.tanggal between ? and ? group by c.venue_id order by score desc, d.positive DESC limit ? ';
        $results = $this->db->query($sql, array($fromDate, $toDate, $limit))->result();
        ...
        $this->load->view('trend_result', $return);
    }

    public function komen($id){
        $sql = 'SELECT * FROM comment WHERE venue_id = ?';
        $results = $this->db->query($sql, array((int)$id))->result();
        ...
        foreach ($data as $value) {
            switch ($value['value']) {
                case 0:
                    array_push($netral, $value);
                    break;
                case 1:
                    array_push($positive, $value);
                    break;
                case -1:
                    array_push($negative, $value);
                    break;
                default:
                    break;
            }
        ...
        $return['max'] = max([count($positive), count($negative), count($netral)]);
        $return['positive'] = $positive;
        $return['netral'] = $netral;
        $return['negative'] = $negative;
        
        $this->load->view('komen', $return);
    }

    public function gambar($id){
        $sql = 'SELECT gambar FROM `venue` WHERE `venue`.`venue_id` = ?';
        $results = $this->db->query($sql, array((int)$id))->result();

        $data = [];

        foreach ($results as $result) {
            $tmp['gambar'] = $result->gambar;

            array_push($data, $tmp);
        }
        $return['data'] = $data;

        $this->load->view('gambar', $return);
    }
}